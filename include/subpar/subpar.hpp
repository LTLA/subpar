 ...
 * For custom schemes, if `SUBPAR_CUSTOM_PARALLEL_NOTHROW` is defined, it will be used if `nothrow_ = true`;
 * otherwise, `SUBPAR_CUSTOM_PARALLEL` will continue to be used.
 *
...
#ifdef SUBPAR_CUSTOM_PARALLEL_NOTHROW
        SUBPAR_CUSTOM_PARALLEL_NOTHROW(num_workers, num_tasks, run_task_range);
#else
        SUBPAR_CUSTOM_PARALLEL(num_workers, num_tasks, run_task_range);
#endif
    } else {
        SUBPAR_CUSTOM_PARALLEL(num_workers, num_tasks, run_task_range);
    }
